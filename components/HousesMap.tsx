 ...
 * so the exact location can't be pinpointed.
 */
export default function HousesMap({ houses }: { houses: House[] }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pts.length) map.fitBounds(group.getBounds().pad(0.6), { maxZoom: 13 });
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200">
      ...
      <p className="bg-slate-50 px-3 py-2 text-xs text-slate-400">
        Pins show the general neighborhood only — exact addresses are shared after booking on PadSplit.
      </p>
    </div>
  );
}
